...
class DungeonAdventure:
    ...
    def create_a_maze(self):
        ...
        maze.place_healing()
        maze.place_vision()
        Label(center_frame, text=maze, width=30, bg='black', fg='yellow', font=("Courier New", 18)).pack()

        player_input = Entry(center_frame, bg='black', fg='yellow', width=10)
        player_input.pack(side=BOTTOM)
        bottom_frame = Frame(root)
        bottom_frame.pack(side=BOTTOM)
        # enter_button = Button(text='Enter', command=get_input)
        root.mainloop()
